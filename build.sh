...
# ----------------------------------------------------
    ...
    TOTVS_DIR="./totvs"
    TOTVS_LICENSESERVER_DIR="${TOTVS_DIR}/licenseserver"
    TOTVS_LICENSESERVER_FILES=(
        "${TOTVS_DIR}/install"
        "${TOTVS_DIR}/installer.jar"
        "${TOTVS_DIR}/license.png"
    )
    ...
    if [[ -n "$CUSTOM_TAG" ]]; then
        ...
        DOCKER_TAG="${DOCKER_USER}/${LICENSESERVER_IMAGE_NAME}:${LICENSESERVER_VERSION}"
    fi
...
# ----------------------------------------------------
    ...
    if [[ "$USING_CUSTOM_BASE" == "true" ]]; then
        ...
        for dir in "${TOTVS_DIR}" "${TOTVS_LICENSESERVER_DIR}" ; do
        ...
        print_verify "Verificando o arquivo em licenseserver/..."

        for file in "${TOTVS_LICENSESERVER_FILES[@]}"; do
            if [ ! -f "$file" ]; then
                if [[ -f "unpack.sh" ]] && [[ -d "packages" ]]; then
                    print_info "License Server binário não encontrado. Tentando extrair..."
                    ./unpack.sh
                fi
            fi
