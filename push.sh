...
# ==============================================================================
# SCRIPT: push.sh
# DESCRIÇÃO: Envia a imagem Docker do LicenseServer para o Docker Hub.
...
# ----------------------------------------------------
    ...
    if [[ -n "$CUSTOM_TAG" ]]; then
        ...
        FULL_TAG="${DOCKER_USER}/${LICENSESERVER_IMAGE_NAME}:${LICENSESERVER_VERSION}"
    fi

    LATEST_TAG="${DOCKER_USER}/${LICENSESERVER_IMAGE_NAME}:latest"

    # Validação de variáveis
    if [[ -z "${LICENSESERVER_VERSION:-}" ]] || [[ -z "${LICENSESERVER_IMAGE_NAME:-}" ]] || [[ -z "${DOCKER_USER:-}" ]]; then
